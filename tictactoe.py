...
class TicTacToe():
    ...
    def score(self, x, y, winner_sign, playing_sign=None, score=0, deep=1.0):
        ...
        if working_board.isThereWinner() is not None:
        ...
        for new_x in range(0, 3):
            for new_y in range(0, 3):
                if working_board.board[-1][new_x][new_y] == 0:
                    temp_score = working_board.score(new_x, new_y,
                                                     winner_sign,
                                                     new_playing_sign,
                                                     temp_score, deep / 2)
    ...
    def play_a_game(self, player=None):
        while self.moves <= 9:
            ...
            self.display()
            if self.isThereWinner() is not None:
                print("You win !")
                break
            else:
                if self.moves == 9:
    ...
    def solve(self, boards, best_moves, x=None, y=None, sign='X'):
        ...
        if ((x is None) and (y is None)):
            for x in range(0, 3):
                for y in range(0, 3):
                    new_boards, new_best_moves = working_board.solve(new_boards,
                                                                     new_best_moves,
                                                                     x, y, sign)
